...
static void set_button_text(CBUTTON *_object, const QString &text)
{
	WIDGET->setText(text);
	WIDGET->calcMinimumSize();
}

static void set_button_picture(CBUTTON *_object)
{
	QPixmap p;
	QIcon icon;
...
static void set_tool_button(CBUTTON *_object, bool set_text, QString text)
{
	QPixmap p;
	QIcon icon;

	if (!set_text)
		text = WIDGET_TOOL->text();

	if (THIS->picture)
	{
		p = *(THIS->picture->pixmap);

		WIDGET_TOOL->setText(text);
		CWIDGET_iconset(icon, p);
...
BEGIN_PROPERTY(CBUTTON_text)
	...
	if (READ_PROPERTY)
		...
		set_button_text(THIS, QSTRING_PROP());
...
BEGIN_PROPERTY(CTOOLBUTTON_text)
	...
	if (READ_PROPERTY)
		...
		set_tool_button(THIS, true, QSTRING_PROP());
...
BEGIN_PROPERTY(CTOOLBUTTON_picture)
	...
	{
		GB.StoreObject(PROP(GB_OBJECT), POINTER(&(THIS->picture)));
		set_tool_button(THIS, false, QString());
	}
